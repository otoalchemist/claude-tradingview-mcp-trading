#!/usr/bin/env node
// ═══════════════════════════════════════════════════════════════════════════
// craig-backtest-pepe.mjs — PEPE-USD Strategy Optimiser
//
// Phase 1: Compares regime timeframes (15m / 30m / 1h / 4h / 6h) with 5m exec
...
//
// Usage:  node craig-backtest-pepe.mjs [lookbackDays]
//   e.g.  node craig-backtest-pepe.mjs 90
// ═══════════════════════════════════════════════════════════════════════════
...
const SYMBOL = "PEPE-USD";

// ── Regime timeframes to test ─────────────────────────────────────────────────
// Include 6h — meme coins often need wider regimes to filter noise
const REGIME_TFS = [
  { label: "15m", ms:      900_000, warmupDays:  50 },
  { label: "30m", ms:    1_800_000, warmupDays: 100 },
  { label: "1h",  ms:    3_600_000, warmupDays: 180 },
  { label: "4h",  ms:   14_400_000, warmupDays: 180 },
  { label: "6h",  ms:   21_600_000, warmupDays: 180 },
];
...
// Current live bot config for PEPE (for comparison)
const CURRENT_BUY_LADDER  = [33, 33, 33, 33];
const CURRENT_SELL_LADDER = [30, 25, 20, 10];
const CURRENT_REGIME_LABEL = "4h";
const CURRENT_REGIME_MS    = 14_400_000;
  ...
  while (endMs > cutoff) {
    ...
    try {
      const res = await fetch(url, {
        headers: { "User-Agent": "craig-backtest-pepe/1.0" },
      ...
      if (!Array.isArray(data)) throw new Error(`Unexpected: ${JSON.stringify(data).slice(0, 100)}`);
...
// ── Aggregate 5m bars into any regime bucket size ─────────────────────────────
...
function buildCrossMap(candles, regimeMs) {
  ...
  for (let i = 1; i < candles.length; i++) {
    ...
    if      (efP <= esP && ef > es) crossMap.set(ct, "golden");   // → SELL
    else if (efP >= esP && ef < es) crossMap.set(ct, "death");    // → BUY
  }
  return crossMap;
}
...
function runSim(execBars, crossMap, regimeMs, buyLadder, sellLadder) {
  ...
  return {
    ...
    totalCrosses: state.regimeCount.buy + state.regimeCount.sell,
...
async function main() {
  console.log(`\n╔══════════════════════════════════════════════════════════════╗`);
  console.log(`║      PEPE-USD  Strategy Optimiser  |  Exec: 5m              ║`);
  console.log(`║      ${LOOKBACK_DAYS}d lookback  ×  5 regime TFs  ×  9×9 ladders        ║`);
  console.log(`╚══════════════════════════════════════════════════════════════╝\n`);
  ...
  if (raw5m.length < 500) {
    console.error(`  ✗ Only ${raw5m.length} bars — check network.`);
    process.exit(1);
  }
  ...
  console.log(`  PEPE: ${firstBar.c.toExponential(3)} → ${lastBar.c.toExponential(3)}  |  Buy & Hold: ${buyHold >= 0 ? "+" : ""}${buyHold.toFixed(2)}%\n`);

  // ── PHASE 1: Regime comparison ─────────────────────────────────────────────
  ...
  for (const r of regimeResults) {
    ...
    const curr   = r.regime === CURRENT_REGIME_LABEL ? " ← current" : "";
    const pnlStr = `${r.pnlPct >= 0 ? "+" : ""}${r.pnlPct.toFixed(2)}%`;
    const vsStr  = `${r.alpha  >= 0 ? "+" : ""}${r.alpha.toFixed(2)}%`;
    const ddStr  = `-${r.maxDrawdown.toFixed(2)}%`;
    console.log(`${star} ${r.regime.padEnd(8)} ${pnlStr.padStart(9)} ${vsStr.padStart(9)} ${ddStr.padStart(9)} ${String(r.trades).padStart(7)} ${String(r.totalCrosses).padStart(9)} ${String(r.regimeCount.buy).padStart(5)} ${String(r.regimeCount.sell).padStart(5)}${curr}`);
  }
  console.log(`  ${"─".repeat(72)}`);
  ...
  for (const rt of topRegimes) {
    ...
    for (const buy of BUY_LADDERS) {
      for (const sell of SELL_LADDERS) {
        ...
        allResults.push({ regime: rt.regime, buyName: buy.name, buyV: buy.v, sellName: sell.name, sellV: sell.v, ...r });
  ...
  // ── Also score the current live config ────────────────────────────────────
  const currRegime = regimeResults.find(r => r.regime === CURRENT_REGIME_LABEL) ?? regimeResults[0];
  const currResult = runSim(execBars, currRegime.crossMap, CURRENT_REGIME_MS, CURRENT_BUY_LADDER, CURRENT_SELL_LADDER);
  ...
  for (let i = 0; i < Math.min(TOP_RESULTS, allResults.length); i++) {
    ...
    const vsStr  = `${r.alpha  >= 0 ? "+" : ""}${r.alpha.toFixed(2)}%`;
    const ddStr  = `-${r.maxDrawdown.toFixed(2)}%`;
    console.log(`${rank} ${r.regime.padEnd(5)} ${r.buyName.padEnd(12)} ${r.sellName.padEnd(12)} ${pnlStr.padStart(9)} ${vsStr.padStart(9)} ${ddStr.padStart(9)} ${String(r.trades).padStart(7)}`);
  }
  console.log(`  ${"─".repeat(80)}`);

  // ── Buy/Sell ladder rankings on the best regime ───────────────────────────
  const bestRegimeLabel = regimeResults[0].regime;
  const bestRegimeRows  = allResults.filter(r => r.regime === bestRegimeLabel);

  console.log(`\n  Buy Ladder Rankings (avg alpha, ${bestRegimeLabel} regime):`);
  ...
  for (const b of buyRanked) {
    ...
    const curr = b.name === "flat-33" ? " ← current" : "";
    console.log(`    ${b.name.padEnd(12)} avg alpha: ${(b.avgAlpha >= 0?"+":"") + b.avgAlpha.toFixed(2)}%  ${bar}${curr}`);
  }

  console.log(`\n  Sell Ladder Rankings (avg alpha, ${bestRegimeLabel} regime):`);
  ...
  for (const s of sellRanked) {
    ...
    const curr = s.name === "front-steep" ? " ← current" : "";
    console.log(`    ${s.name.padEnd(12)} avg alpha: ${(s.avgAlpha >= 0?"+":"") + s.avgAlpha.toFixed(2)}%  ${bar}${curr}`);
  }

  // ── Summary ────────────────────────────────────────────────────────────────
  const best = allResults[0];
  console.log(`\n${"═".repeat(80)}`);
  console.log(`  CURRENT CONFIG vs BEST FOUND`);
  console.log(`${"═".repeat(80)}`);
  console.log(`  Current  (${CURRENT_REGIME_LABEL} / ${CURRENT_BUY_LADDER.join(",")} buy / ${CURRENT_SELL_LADDER.join(",")} sell):`);
  console.log(`    P&L ${currResult.pnlPct>=0?"+":""}${currResult.pnlPct.toFixed(2)}%  |  Alpha ${currResult.alpha>=0?"+":""}${currResult.alpha.toFixed(2)}%  |  Max DD -${currResult.maxDrawdown.toFixed(2)}%  |  ${currResult.trades} trades`);
  console.log(`  Best     (${best.regime} / ${best.buyName} / ${best.sellName}):`);
  ...
  console.log(`  Buy & Hold: ${buyHold>=0?"+":""}${buyHold.toFixed(2)}%\n`);
  ...
  // ── Save JSON ─────────────────────────────────────────────────────────────
  const outFile = "backtest-pepe-opt-results.json";
  writeFileSync(outFile, JSON.stringify({
    symbol: SYMBOL, execGran: "5m", lookbackDays: LOOKBACK_DAYS,
    period: { from: new Date(firstBar.t).toISOString(), to: new Date(lastBar.t).toISOString(), days: +periodDays.toFixed(0) },
    buyHoldPct: +buyHold.toFixed(2),
    currentConfig: {
      regime: CURRENT_REGIME_LABEL, buy: CURRENT_BUY_LADDER, sell: CURRENT_SELL_LADDER,
      pnlPct: +currResult.pnlPct.toFixed(2), alphaPct: +currResult.alpha.toFixed(2),
      maxDrawdownPct: +currResult.maxDrawdown.toFixed(2), trades: currResult.trades,
    },
    phaseOne: {
      results: regimeResults.map(r => ({
        regime: r.regime, pnlPct: +r.pnlPct.toFixed(2), alphaPct: +r.alpha.toFixed(2),
        maxDrawdownPct: +r.maxDrawdown.toFixed(2), trades: r.trades,
        totalCrosses: r.totalCrosses, regimeCount: r.regimeCount,
      })),
    },
    phaseTwo: {
      top20: allResults.slice(0, 20).map(r => ({
        regime: r.regime, buyLadder: r.buyName, buyV: r.buyV,
        sellLadder: r.sellName, sellV: r.sellV,
        pnlPct: +r.pnlPct.toFixed(2), alphaPct: +r.alpha.toFixed(2),
        maxDrawdownPct: +r.maxDrawdown.toFixed(2), trades: r.trades,
      })),
      buyLadderRanking:  buyRanked.map(b => ({ name: b.name,  avgAlpha: +b.avgAlpha.toFixed(2) })),
      sellLadderRanking: sellRanked.map(s => ({ name: s.name, avgAlpha: +s.avgAlpha.toFixed(2) })),
    },
    recommendation: {
      regimeTF: best.regime,
      buyLadder:  { name: best.buyName,  v: best.buyV  },
      sellLadder: { name: best.sellName, v: best.sellV },
      pnlPct: +best.pnlPct.toFixed(2), alphaPct: +best.alpha.toFixed(2),
      maxDrawdownPct: +best.maxDrawdown.toFixed(2), trades: best.trades,
    },
  }, null, 2));
